...
const AnalysisResult = ({ result, structured, onReset }) => {
    ...
    const buildReportText = () => {
        const lines = [
            'NeuroGaurd AI - Patient Screening Report',
            '========================================',
    ...
    const handleDownloadReport = () => {
        ...
        link.href = url;
        link.download = `neurogaurd-report-${detectedOrgan}-${timeStamp}.txt`;
    ...
    const buildPdfDocument = () => {
        ...
        doc.setFontSize(14);
        doc.text('NeuroGaurd AI Report', margin, margin);
    ...
    const handleDownloadPdf = () => {
        const doc = buildPdfDocument();
        const timeStamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-');
        doc.save(`neurogaurd-report-${detectedOrgan}-${timeStamp}.pdf`);
    };
    ...
    return (
        ...
        >
            <div className="card-redesign h-full flex flex-col overflow-hidden !p-0">

                {/* Header */}
                <div className="bg-[var(--bg-primary)] px-6 py-5 border-b border-[var(--border-subtle)] flex items-center justify-between">
                    <div className="flex items-center gap-3 text-[var(--text-primary)]">
                        <Sparkles size={20} className="text-[var(--accent-primary)]" />
                        <h3 className="text-lg font-bold tracking-tight">AI ANALYSIS REPORT</h3>
                    </div>

                    {!isTyping && (
                        <button
                            onClick={handleCopy}
                            className={`p-2 rounded-xl transition-all flex items-center gap-2 text-xs font-bold ${isCopied ? 'bg-emerald-500/20 text-emerald-400' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)] border border-[var(--border-subtle)]'}`}
                        >
                            {isCopied ? <CheckCircle2 size={14} /> : <Copy size={14} />}
                            <span className="hidden sm:inline">{isCopied ? 'COPIED' : 'COPY REPORT'}</span>
                        </button>
                    )}
                </div>
                ...
                >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
                        ...
                        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-5 text-left">
                            <p className="text-xs uppercase tracking-widest text-[var(--text-secondary)] mb-3 font-semibold">Diagnosis Confidence</p>
                            <p className="text-2xl font-bold text-[var(--accent-primary)] tracking-tight">{confidenceValue}</p>
                            <div className="mt-4 flex flex-wrap gap-2">
                                <span className="px-3 py-1 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-full text-[10px] text-[var(--text-secondary)] font-mono">ORGAN: {detectedOrgan.toUpperCase()}</span>
                                <span className={`px-3 py-1 border rounded-full text-[10px] font-bold uppercase tracking-wider ${urgencyClass}`}>
                                    URGENCY: {urgencyLevel}
                                </span>
                            </div>
                        </div>
